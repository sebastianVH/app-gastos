...
class Vista():
    def __init__(self, main: Controller , objeto_base: Abmc):
        ...
        menu_archivo.add_command(label="Cambiar base", command=lambda: self.objeto_base.cambiar_db(self.tree))
        ...
        botonagregar = Button(self.master,text="Agregar", width=10, command=lambda: self.objeto_base.agregar(self.fecha,self.tipo,self.monto,self.descripcion,self.tree, balance_label))
        botonagregar.place(x=370, y=0)
        
        botonmostrar = Button(self.master,text="Actualizar",width=10,command=lambda: self.objeto_base.mostrar(self.tree, balance_label))
        botonmostrar.place(x=370, y=30)
        
        botonmodificar = Button(self.master, text="Editar", width=10, command=lambda: self.objeto_base.modificar(self.fecha,self.tipo,self.monto,self.descripcion,self.tree, balance_label))
